...
\begin{tikzpicture}[scale=1.5]
    ...
    % Car middle point
    \coordinate (m) at (\cl*1, \bh*0.2);
    \filldraw (m) circle (2pt) node[below right, xshift=-0.5cm, yshift=-0.3cm] {Car ($\mathcal{C}$)};
    % Coordinate frames
    \draw[thick,->, red] (m) -- ++(0.7,0,0) node[anchor=north east]{$x$};
    \draw[thick,->, black!35!green] (m) -- ++(0,0,-0.7) node[anchor=west]{$y$};
    \draw[thick,->, blue] (m) -- ++(0,0.7,0) node[anchor=south]{$z$};

    % IMU middle point (random)
    \begin{scope}[rotate=15]
        \coordinate (mi) at (\cl*0.2, \bh*0.5);
        \filldraw (mi) circle (2pt) node[below] {\glsentryshort{imu} ($\mathcal{I}$)};
        % Coordinate frames
        \draw[thick,->, red] (mi) -- ++(0.7,0,0) node[anchor=north east]{$x$};
        \draw[thick,->, black!35!green] (mi) -- ++(0,0,-0.7) node[anchor=west]{$y$};
        \draw[thick,->, blue] (mi) -- ++(0,0.7,0) node[anchor=south]{$z$};
    \end{scope}
