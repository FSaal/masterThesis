...
\begin{tikzpicture}[scale=1]
    ...
    \begin{scope}[scale=0.7, xshift=\rl*0.5 cm, yshift=1.9 cm, rotate=\ra]
        ...
        \draw[blue, thick, ->] (m) -- ++(-1.3,0) node[anchor=east]{$a_\mathrm{at}$};
        % Gravity vectors
        % Length of g vector
        \def\gl{2.8}
        ...
        \draw[red, semithick, ->] (m) -- ++(-\gxl,0) node[anchor=south]{$g_\mathrm{x}$};
        \draw[red, semithick, ->] (m) -- ++(0,-\gzl) node[anchor=west]{$g_\mathrm{z}$};
