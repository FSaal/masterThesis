...
\itodo{Also add ZED 2i IMU and compare (probably table similar to lidar)}
...
\itodo{Add vertical FOV ranges to table (e.g. 0 to 90 deg for BP or -15 to 25 or whatever velodyne has)}
\begin{table}[ht]
	\centering
...
\subsection{Odometry}
\todoin{I think this can be ommited? (functioning already explained in background and velocity calculation in methods)}
...
\end{figure}
\itodo{IMU placement not really important, should just be static}
...
\[ c = h - \frac{z}{\tan \alpha} \]
\itodo{Lidar pitch angle at which most point hit area before ramp can be calculated, explaination here using sketch \ref{fig:tikz_lidar_mount}}
...
\todoin{\begin{itemize}
	...
	\item Problem with speed limit and odometer (no odometer when driving ramp up)
