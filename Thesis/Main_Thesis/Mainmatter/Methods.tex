...
\itodo{Enter filter coefficient (or should this be done in results?)}
...
The average angle of the ramp is calculated by
\itodo{Explain method to estimate average ramp angle}
...
Since the \gls{ransac} algorithm provides the normal vector of the plane, the angle of the detected plane can be compared to the normal vector of the ground vector, which is $\left(\begin{array}{lll} 0 & 0 & 1 \end{array}\right)^{\intercal}$ after the calibration.
...
\iimprov{This section needs improvement}
Beside the angle, also the width of the plane is being calculated and checked if it surpasses a certain threshold, to make sure that the plane is a drivable ramp for cars and not e.g. a small ramp for wheelchairs.
The width is calculated by taking the mean of the $m$ most left and right points respectively and calculating the absolute difference between them.
Furthermore, the distance to the ramp is estimated by taking the $n$ nearest points of the ramp plane and calculating their mean.
If all the conditions have been fulfilled, the estimated angle and distance from the car front to the beginning of the ramp are returned.\\
...
\begin{figure}[htb]
	...
	\begin{subfigure}{.47\linewidth}
		...
		\caption{Augmented image}
	\end{subfigure}
	\caption[TODO]{Couple of examples of the annotated images used for the training. On the left side the original image can be seen, on the right side the augmented image.}
...
Different properties of the ramp can now be calculated, similarly as described in \cref{sssec:ramp_properties}.
