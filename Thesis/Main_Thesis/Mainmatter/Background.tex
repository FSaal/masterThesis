...
\begin{figure}[htb]
	...
	\caption{Micro structure of a \gls{mems} accelerometer (TODO: Better image)}
...
Hence the elimination of the bias is necessary to get reliable estimations of the the orientation, velocity or position, which are calculated by integrating the angular velocity or the accelerometer measurements.\\
...
The integration of the white noise leads to a random walk \todo{How to fix white noise, allen variance?}
...
Random walk angle errors are usually the hardest to correct and are the reason, why the gyroscope can not be trusted over a long period of time.
