...
A quaternion is a complex number with three imaginary parts
\begin{equation}
...
\end{equation}
Each rotation is uniquely described by one rotation, but each orientation ca be described by two unit quaternions with opposite signs.
\begin{equation}
...
\begin{equation}
	...
	\end{bmatrix}
\end{equation}
\iimprov{Improve quaternion text and add new stuff}
...
The measurements $\hat{\vb{a}}$ of a \gls{mems} accelerometer are not free of noise and can be modeled in a simplified way as
\begin{equation}
...
\iimprov{Add source for formula}
