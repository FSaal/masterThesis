...
\iimprov{Remove eigenvector stuff, not important, but write something else}
...
\iimprov{Explain when/how singularity occurs}

\subsubsection{Quaternions}
\iimprov{Check if notation is correct (bold and stuff)}
...
More information about quaternions can be found in \cite{Trawny2005}.\\
...
\begin{equation}
	...
	\left[
		...
		\right]
\end{equation}
\iimprov{How to calculate matrix?}
...
\end{equation}
The measurement differs from the true angular velocity $\boldsymbol{\omega}$ due to the bias vector $\vb{b}_\mathrm{g}$ and the random noise vector $\vb{n}_\mathrm{g}$, which has a mean of zero \cite{Lefferts1982}.
