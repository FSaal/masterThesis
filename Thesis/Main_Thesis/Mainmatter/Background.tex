...
But this comes at the cost of a worse angle drift which increases from 0.01 to 0.1 deg/h for optical gyroscopes to 10 deg/h \todo{check what type of deg/h there are, daytoday vs in-run} for MEMS-IMUs.
...
\subsection{Odometer?}
\itodo{Probably brief explaination}
...
This also explains the naming of the two types, the FIR filter "forgets" past values, whereas the IIR filter uses the previous estimate and thus theoretically takes all past values into account.\\
