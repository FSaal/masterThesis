...
Besides the road grade, the vehicle mass is often also unknown and estimated as well, using common sensors of heavy-duty vehicles \cite{Sahlholm2010}  or \itodo{what sensors does this paper use?} \cite{Maleej2014}.
...
ANOTHER COMPL FILTER PAPER also uses a complementary filter, but fuses the estimated angle. from the accelerometer and gyroscope instead of the quaternions.
\iquest{I did not use most methods mentioned in the papers (e.g. Kalman filter). How to justify this?}
\isug{Performance\\ And maybe implement something simple (from a package)}
